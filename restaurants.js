/**
 * Restaurant integration using Google Places API
...
export async function fetchRestaurantsForCity(city) {
  ...
  if (!GOOGLE_API_KEY || GOOGLE_API_KEY === 'YOUR_API_KEY_HERE') {
    throw new Error(
      ...
      '4. Replace YOUR_API_KEY_HERE in restaurants.js'
    );
  }
  ...
  try {
    ...
    // Use Google Places Text Search API for restaurants with CORS proxy
    // Make the search more location-specific by including the city name more explicitly
    const placesUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=restaurants+in+${cleanCity}+${cleanCity}&key=${GOOGLE_API_KEY}&type=restaurant&fields=name,rating,types,formatted_address,price_level,place_id`)}`;
    ...
    if (!placesData.results || placesData.results.length === 0) {
      throw new Error(`No restaurants found in ${city}. Try a larger city!`);
    }

    // Transform Google Places data to our format and filter for local restaurants
